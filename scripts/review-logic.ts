...
const report='# 추천 설계 추가 진단\n\n2026-09-22 재실행. 추천 정책의 민감도와 설명 방식의 한계를 확인하는 진단이다. `node --import tsx scripts/review-logic.ts`로 재현한다. 원본 CSV를 읽으며 수정하지 않는다. 현재 정책을 채택할 근거와 반대 사례를 함께 찾는다.\n\n'
...
+'\n\n## 설명의 정확성과 유용성을 구분\n\n균등 기준 1위의 최대 기여 지표와 2위 대비 최대 점수 차이를 만든 지표가 '+explanations.length+'/'+meaningful.length+'조건에서 다르다. 이전 UI는 최대 기여 지표만 보여줬다. 현재 상세는 사용자가 우선한 지표의 상대 위치를 문장으로 설명하고 실제 비중·비교 집단을 펼쳐볼 수 있다. 이는 후보의 특징 설명이며 2위보다 앞선 원인을 직접 분해하는 설명은 아니다. 후보 간 기여 차이는 비교 메모 생성 함수에 남아 있지만 현재 화면의 대표 설명으로 사용하지 않는다. 이 값은 과거 설명 선택 규칙의 한계를 재현한 값이지 현재 오류 건수가 아니다.\n\n'
