...
writeFileSync('docs/EVALUATION.md','# 추천 방식 비교와 민감도\n\n`npm run analyze`로 원본 전체와 동일한 추천 함수를 사용해 재생성한다. 현재 균등 비교(25/25/25/25)의 실험이다. 정답 성과가 없으므로 정확도·매출 개선 실험이 아니다.\n\n'
  ...
  +'\n\n## 균등 기준 예시의 실제 후보\n\n뷰티·패션, 마이크로, 2,000,000원. 이 분석 표의 총점은 소수 첫째 자리, 구성요소는 둘째 자리까지 표시한다. 화면은 추천 이유와 적용 비중을 우선 보여주며 정렬은 반올림 전 점수를 사용한다. 온보딩은 제품 알리기 목표와 조회수 우선으로 시작하고 사용자가 변경할 수 있다. 이 표의 균등 기준은 방식 비교를 위한 실험 설정이다.\n\n| ID | 이름 | 플랫폼 | 점수 | 참여 기여 | 조회 기여 | 평점 기여 | 경험 기여 |\n|---|---|---|---:|---:|---:|---:|---:|\n'
