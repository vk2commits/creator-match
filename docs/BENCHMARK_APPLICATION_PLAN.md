...
작성·출처 재확인: 2026-09-22. 상태: **전체 개선 계획 + 세 차례 적용**. 전체 구현 완료나 사용자 검증 결과가 아니다. [1차 분석·근거·문의 연결](UNIFIED_ANALYSIS.md), [2차 온보딩 축소](ONBOARDING_REVIEW.md), [3차 검색 자료 범위·목록 정리](SEARCH_EVIDENCE.md)를 적용했다. 캠페인별 성과·새 환경·실제 모델/다운로드 검증과 사용자 최종 검토·제출은 남아 있다.
...
- 포함은 기본 OR이며 3차에서 ‘하나 이상 포함/모두 포함’ 선택을 추가했다. ‘뷰티 또는 직장인’처럼 넓어지는 조합은 사용자가 모두 포함으로 좁힐 수 있다. 콘텐츠 주제와 스타일을 별도 분류하는 고도화는 추가 검토다.
...
현재 200명 중 콘텐츠 예시는 5명에게만 있다. 3차에서 `키워드 확인 / 자료 확인 필요 / 검색에서 제외`로 분리했다. 제외 사유도 제외 키워드 발견과 등록 범위 내 포함 키워드 미확인으로 구분한다. 자료가 없는 나머지를 부적합으로 분류하거나, 단어가 없다는 이유로 채널 전체가 안전하다고 단정하지 않는다.
