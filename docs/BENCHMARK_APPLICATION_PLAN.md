...
작성·출처 재확인: 2026-09-22. 상태: **사용자 검토용 개선 계획**. 이 문서는 개선의 전체 범위이며 구현 완료나 사용자 검증 결과가 아니다. 이후 첫 단계인 분석·근거·문의 연결을 적용했다. [적용 결과와 검증 범위](UNIFIED_ANALYSIS.md)를 별도로 확인한다. 온보딩 축소·검색의 자료 미확인 분리는 아직 남아 있다.
...
이것은 제안 흐름이며 현행 4단계 구현의 설명이 아니다. 실제 가입은 과제의 선행 조건이 아니므로 새 인증 기능을 넣지 않는다. 향후 서비스에서는 가입 직후 이 캠페인 시작 화면으로 들어오도록 연결할 수 있다.
...
현행 네 프리셋과 직접 조절을 유지한다. 첫 검토에서는 목표와 연결된 프리셋·이유를 보여주고 `비중 직접 조절`을 열 수 있게 한다. 별도 필수 온보딩 단계로 강제하지 않는다.
