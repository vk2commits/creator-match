...
export function CollaborationEditor({creator:c,record,brief,reason,onSave,onClose,onReport,initialResults=false,initialSent=false}:{creator:Creator;record:WorkRecord;brief:Brief;reason:string;onSave:(record:WorkRecord)=>void;onClose:()=>void;onReport:()=>void;initialResults?:boolean;initialSent?:boolean}){
  ...
  const advance=()=>{
    if(draft.stage==='draft'||composing){
      try{const next=simulateSend(draft,text,new Date().toISOString());setSending(true);timer.current=setTimeout(()=>{onSave(next);setDraft(next);setSending(false);setComposing(false);setHistory(true);setResult(false);setStatus(c.name+'님의 예시 수신함으로 문의를 보냈어요. 실제 메일·DM은 발송되지 않습니다.');},700);}catch(e){setErrors([e instanceof Error?e.message:'문안을 확인해 주세요.']);}return;
    }
  ...
  return <Modal title={c.name+' · 문의와 협업'} onClose={close} wide><div className="workflow-editor">
    ...
    <div className="stage-guide"><span className="section-kicker">{STAGES.find(s=>s.id===draft.stage)?.label}</span><h3>{composing?'추가 문의를 작성하세요':guide.title}</h3><p>{composing?'이전 문안을 수정해 다시 보낼 수 있어요. 보낸 내역과 협업 상태는 유지됩니다.':guide.description}</p></div>
    ...
    {history?<><SentMessageHistory record={draft}/><button className="secondary-button" onClick={()=>{setHistory(false);setResult(false);setComposing(true);}}>추가 문의 작성</button></>:result?<OutcomeFields value={draft.outcome} onChange={outcome=>setDraft({...draft,outcome})}/>:<>
      {(draft.stage==='draft'||composing)&&<><div className="recipient-bar"><span>받는 사람</span><strong>{c.name}</strong><small>예시 수신함 · 발송 시연</small></div><section className="inquiry-draft"><div className="section-heading"><h3>보낼 문의 문안</h3><button className="ai-button" disabled={sending} onClick={()=>setShowAI(!showAI)}>✦ AI로 다듬기</button></div><textarea aria-label="보낼 문의 문안" disabled={sending} value={text} onChange={e=>setDraft({...draft,message:e.target.value})}/><div className="form-actions"><button className="text-button" onClick={()=>setDraft({...draft,message:''})}>현재 조건으로 다시 채우기</button><button className="secondary-button" onClick={async()=>{try{await navigator.clipboard.writeText(text);setStatus('문안을 복사했습니다. 실제 메일이나 DM에 붙여 넣으세요.');}catch{setStatus('문안을 직접 선택해 복사해 주세요.');}}}>문안 복사</button>{draft.email&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(draft.email)&&<a className="text-button" href={'mailto:'+encodeURIComponent(draft.email)+'?subject='+encodeURIComponent(campaignOf(brief).name+' 협업 문의')+'&body='+encodeURIComponent(text)}>이메일 초안 열기 ↗</a>}</div></section>{showAI&&<AssistantPanel mode="outreach" context={text} onApply={message=>{setDraft({...draft,message});setShowAI(false);setStatus('AI 문안을 적용했습니다. 보내기 전 내용을 확인해 주세요.');}}/>}<details className="contact-details"><summary>문의에 포함할 제작 조건</summary>{terms}</details></>}
      {!composing&&draft.stage==='contacted'&&<div className="waiting-card"><strong>{draft.demoSentAt?'문의 보내기를 체험했어요.':'답변이 도착하면 다음으로 이어가세요.'}</strong><p>{draft.demoSentAt?'아래 ‘답변 받았어요’를 누르면 견적과 제작 조건을 기록할 수 있어요.':'아직 답변이 없다면 기록을 저장하고 목록에서 다른 후보를 검토하세요.'}</p></div>}
      {!composing&&['negotiating','active','complete'].includes(draft.stage)&&<section className="terms-section"><h3>{draft.stage==='negotiating'?'답변과 받은 견적':'최종 협업 조건'}</h3><div className="field-grid"><AmountField label="받은 견적 (원)" value={draft.quotedCost} onChange={quotedCost=>setDraft({...draft,quotedCost})}/><div><AmountField label="합의 비용 (원)" value={draft.agreedCost} onChange={agreedCost=>setDraft({...draft,agreedCost})}/>{draft.quotedCost!==null&&<button className="text-button quote-copy" onClick={()=>setDraft({...draft,agreedCost:draft.quotedCost})}>받은 견적으로 채우기</button>}</div></div>{terms}</section>}
      ...
      <details className="contact-details"><summary>{draft.stage==='draft'?'외부에서 직접 연락했나요?':'연락처·실제 문의 날짜'}</summary><p className="data-caption">메일이나 DM으로 직접 보냈다면 날짜와 연락처를 기록하세요.</p>{contactFields}{draft.stage==='draft'&&<button className="secondary-button" disabled={!draft.contactedAt} onClick={()=>{try{const next=recordExternalInquiry(draft,text,record);if(save(next)){setHistory(true);setResult(false);setStatus('외부에서 보낸 문의를 기록했습니다.');}}catch(e){setErrors([e instanceof Error?e.message:'문의 날짜를 확인해 주세요.']);}}}>외부에서 보낸 문의 기록</button>}</details>
    </>}
    ...
    {!leaving&&<div className="editor-footer"><button className="secondary-button" disabled={sending} onClick={()=>{if(save())onClose();}}>저장하고 닫기</button><button className="primary-button" disabled={sending} onClick={result?()=>{if(save())onReport();}:advance}>{sending?'문의 보내는 중…':result?'저장하고 성과 리포트 보기':composing?'추가 문의 보내기 · 시연':guide.action}</button></div>}
  </div></Modal>;
}
