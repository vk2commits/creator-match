...
export function Onboarding({initial,onComplete,onCancel,campaignNav}:{initial?:Brief;onComplete:(brief:Brief)=>void;onCancel?:()=>void;campaignNav?:ReactNode}){
 ...
 <main className={'intake-main '+(phase==='describe'?'intake-start':'intake-review')}>
  ...
  {phase==='describe'?<div className="intake-opening"><section className="intake-intro"><p className="section-kicker">새 캠페인</p><h1 ref={heading} tabIndex={-1}>어떤 제품을<br/>누구에게 소개할까요?</h1><p>정해둔 내용을 편하게 적어주세요.<br/>후보를 찾을 조건으로 정리해 드릴게요.</p><ol className="intake-value"><li><Icon name="search" size={18}/><span><strong>후보 찾기</strong>예산과 조건에 맞는 크리에이터</span></li><li><Icon name="reaction" size={18}/><span><strong>적합 이유 확인</strong>우리 제품과 연결되는 콘텐츠 근거</span></li><li><Icon name="send" size={18}/><span><strong>문의 준비</strong>제작 제안과 확인할 질문</span></li></ol></section><BriefBuilder text={text} onText={setText} onGenerate={source=>{setValue(v=>setupFromText(source,v));setReviewed(true);setSubmitted(false);setPhase('campaign');}} onManual={()=>{setReviewed(true);setPhase('campaign');}} onReturn={reviewed?()=>setPhase('campaign'):undefined}/></div>:<>
   <div className="intake-review-heading"><div><p className="section-kicker">{stepIndex+1} / 3 · {initial?'캠페인 설정':'새 캠페인'}</p><h1 ref={heading} tabIndex={-1}>{titles[phase]}</h1><p>{descriptions[phase]}</p></div><button className="text-button" onClick={()=>setPhase('describe')}>설명으로 다시 정리</button></div>
   {phase==='campaign'&&value.extracted.length>0&&<p className="extraction-note"><Icon name="check" size={15}/>설명에 적은 내용을 채워뒀어요. 다른 부분만 고쳐주세요.</p>}
