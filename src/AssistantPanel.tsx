...
export function AssistantPanel({mode,context,onApply}:{mode:AssistantMode;context:string;onApply?:(text:string)=>void}){
  ...
  return <section className="assistant-panel"><div className="assistant-heading"><span className="ai-symbol">✦</span><div><h3>{analysis?'협업 결과를 한 문단으로 정리하세요':'문의 문안을 다듬어 보세요'}</h3><p>{analysis?'입력한 조회수와 집행비를 읽기 쉽게 정리합니다.':'제품·예산·제작 조건을 가져왔어요. 원하는 말투를 골라주세요.'}</p></div><span className="ai-state">AI</span></div>
    ...
    <p role="status">{status}</p><p className="assistant-demo-foot">적용 전에 문안을 수정할 수 있어요.</p>
  </section>;
}
