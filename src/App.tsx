...
import {FitAnalysis,CampaignFitSummary} from './FitAnalysis';
...
import {campaignOf,EMPTY_CAMPAIGN,GOALS,emptyWork} from './campaign';
...
import { STORAGE_KEY, candidateStatus, criteriaOf, formatBudget, readSession, toggleCompared } from './experience';
...
function Onboarding({initial,onComplete,onCancel}:{initial?:Brief;onComplete:(brief:Brief)=>void;onCancel?:()=>void}) {
  ...
  return <div className="onboarding"><header className="onboarding-header"><span className="wordmark"><span className="brand-sign">c<span>m</span></span>creator match</span>{onCancel&&<button className="text-button" onClick={onCancel}>변경 없이 돌아가기</button>}</header><main className="onboarding-main"><section className="welcome"><div className="step-label">캠페인 설정 <span>0{step} / 04</span></div><h1>{titles[step-1]}</h1><p>{descriptions[step-1]}</p><ol className="steps">{steps.map((label,i)=><li key={label} className={step===i+1?'current':step>i+1?'done':''}><span>{step>i+1?'✓':i+1}</span>{label}</li>)}</ol></section><section className={'setup-pane step-'+step} aria-label={steps[step-1]}>
    {step===1?<><div className="section-heading"><h2>이번 캠페인을 알려주세요.</h2>{!showAI&&<button className="ai-button" onClick={()=>setShowAI(true)}>✦ AI로 초안 만들기</button>}</div>{showAI?<BriefBuilder initial={draft} onClose={()=>setShowAI(false)} onApply={value=>{setDraft(value);setShowAI(false);setStep(2);}}/>:<form className="brief-form" onSubmit={e=>{e.preventDefault();if(campaign.name.trim()&&campaign.product.trim())setStep(2);}}><label className="field-label">캠페인명<input required maxLength={80} value={campaign.name} placeholder="예: 가을 립틴트 출시" onChange={e=>updateCampaign({...campaign,name:e.target.value})}/></label><label className="field-label">제품과 알리고 싶은 점<textarea required maxLength={1500} value={campaign.product} placeholder="예: 한 번만 발라도 선명한 색이 오래가는 립틴트" onChange={e=>updateCampaign({...campaign,product:e.target.value})}/></label><fieldset className="goal-options"><legend>이번 캠페인의 목표</legend>{GOALS.map(g=><label key={g.id} className={campaign.goal===g.id?'selected':''}><input type="radio" name="goal" checked={campaign.goal===g.id} onChange={()=>setDraft({...draft,campaign:{...campaign,goal:g.id},priority:g.priority,customWeights:undefined})}/><span><strong>{g.label}</strong><small>{g.detail}</small></span></label>)}</fieldset><div className="onboarding-actions">{!initial&&<button type="button" className="text-button" onClick={()=>onComplete({input:{...DEFAULT_INPUT,categories:[...DEFAULT_INPUT.categories]},priority:'balanced',campaign:{name:'가을 립틴트 출시',product:'한 번만 발라도 선명한 색이 오래가는 립틴트',goal:'awareness',targetCustomer:'출근 준비 시간을 줄이고 싶은 직장인',customerNeed:'routine'}})}>예시 캠페인으로 시작</button>}<button className="primary-button" type="submit">타깃 고객 정하기<Icon name="arrow" size={16}/></button></div></form>}</>
...
function Profile({creator:c,all,brief,onClose,selected,onToggle,onInquiry,onCampaign,hasWork}:{creator:Creator;all:Creator[];brief:Brief;onClose:()=>void;selected:boolean;onToggle:()=>void;onInquiry:()=>void;onCampaign:(campaign:Campaign)=>void;hasWork:boolean}) {
  const item=scoreCreator(c,all,'cohort',criteriaOf(brief).weights),story=matchStory(item,brief),known=hasKnownBudget(c);
  return <Modal title="캠페인 적합 분석" onClose={onClose}><div className="profile-content"><div className="profile-identity"><Identity creator={c}/><a className="secondary-button" target="_blank" rel="noreferrer" href={channelHref(c.id,DEMO_SESSION)}>채널 보기 ↗</a></div>
    <FitAnalysis creator={c} campaign={campaignOf(brief)} onCampaign={onCampaign}/>
    <section className="selection-evidence"><h3>{story.eligible?'추천 목록에 포함된 이유':'현재 조건에서 확인할 점'}</h3><p className="evidence-lead">{story.summary}</p><div className="fit-reasons"><div><span>후보 조건</span><p>{story.category} 팔로워는 {numberText(c.followers)}명입니다.</p></div><div><span>예산</span><p>{story.budget}</p></div>{story.next&&<div><span>지표의 차이</span><p>{story.next}</p></div>}</div></section>
...
function Compare({selected,all,brief,onClose,onInquiry,work}:{selected:Creator[];all:Creator[];brief:Brief;onClose:()=>void;onInquiry:(id:string)=>void;work:WorkRecords}) {
  const rows:{label:string;value:(c:Creator)=>string;numeric?:(c:Creator)=>number}[]=[
    {label:'현재 조건',value:c=>candidateStatus(c,brief)},
    {label:'캠페인 적합 분석 · AI 시연',value:c=>{const fit=campaignFit(c,campaignOf(brief));return fit.headline+' '+fit.summary;}},
    {label:'추천 지표의 근거',value:c=>matchStory(scoreCreator(c,all,'cohort',criteriaOf(brief).weights),brief).summary},
...
export default function App() {
  ...
  const openInquiry=(id:string)=>{decide(id,'contact');setWork(w=>w[id]?w:{...w,[id]:emptyWork()});setDetail(null);setModal(null);setWorkflowTarget(id);setView('workflow');};
  ...
  return <div className="app"><a className="skip-link" href="#main">결과로 바로가기</a><aside className="sidebar"><a className="wordmark" href={DEMO_SESSION?'?demo':'./'}><span className="brand-sign">c<span>m</span></span><span>creator<br/>match</span></a><span className="nav-label">캠페인 작업실</span><nav><button className={view==='discovery'?'active':''} onClick={()=>setView('discovery')}><Icon name="search"/>크리에이터 찾기</button><button className={view==='saved'?'active':''} onClick={()=>setView('saved')}><Icon name="bookmark"/>저장한 후보 <span>{selected.length}</span></button><button className={view==='sent'?'active':''} onClick={()=>setView('sent')}><Icon name="send"/>보낸 문의 <span>{sentCount(work)}</span></button><button className={view==='workflow'?'active':''} onClick={()=>setView('workflow')}><Icon name="history"/>협업 관리</button><button className={view==='report'?'active':''} onClick={()=>setView('report')}><Icon name="views"/>성과 리포트</button></nav><div className="sidebar-bottom"><span className="demo-dot"/>Creator Match {storageError&&<p role="alert">저장이 제한되어 새로고침하면 초기화됩니다.</p>}</div></aside>
    <div className="main-shell"><header className="topbar"><span>워크스페이스 <span className="slash">/</span> {campaignOf(brief).name}</span><div className="header-actions"><a className="text-button" href="#setup">캠페인 설정</a></div></header><main id="main">
      {view==='workflow'||view==='report'||view==='sent'?data?<Workflow key={view} all={data} contacts={contacts} work={work} brief={brief} notes={notes} report={view==='report'} sent={view==='sent'} initialCreatorId={workflowTarget} onConsumeTarget={()=>setWorkflowTarget(null)} onReport={()=>setView('report')} onExplore={()=>setView('discovery')} onUpdate={(id,value)=>{setWork(w=>({...w,[id]:value}));setToast('협업 기록을 저장했습니다.');}}/>:<div className="empty-state" role="status"><h2>{loadError||'후보를 불러오고 있어요.'}</h2>{loadError&&<button className="primary-button" onClick={()=>setReload(x=>x+1)}>다시 불러오기</button>}</div>:<>
      ...
      <p className="list-context">{view==='saved'?'후보를 비교하거나, 한 명씩 문의하고 협업 기록을 이어가세요.':tab==='quote'?'분야·팔로워 조건은 맞지만 비용 정보가 없습니다. 견적을 확인한 뒤 검토하세요. 평균 조회수순으로 표시합니다.':result.matched.length?'참고 협업비는 과거 평균입니다. 최종 견적은 문의해 주세요.':''}</p>
      <div className="sr-only" role="status">추천 후보 {result.matched.length}명, 비용 미확인 {result.needsQuote.length}명. {priority.label} 기준.</div>
      {visible.length>0?<><div className="candidate-cards">{visible.slice(0,limit).map((c,index)=>{const item=result.matched.find(x=>x.creator.id===c.id)??scoreCreator(c,data!,'cohort',priority.weights),story=matchStory(item,brief);return <article className={'match-card '+(selectedIds.includes(c.id)?'is-selected':'')} key={c.id} aria-label={c.name+' 후보'}><div className="match-card-top"><div className="rank-identity">{view==='discovery'&&sort==='recommended'&&tab==='matched'&&<span className="candidate-rank">{index+1}</span>}<Identity creator={c}/></div>{view==='saved'?<button className={'candidate-save '+(comparedIds.includes(c.id)?'chosen':'')} aria-label={c.name+(comparedIds.includes(c.id)?' 비교표에서 빼기':' 비교표에 추가')} aria-pressed={comparedIds.includes(c.id)} onClick={()=>compareToggle(c.id)}><Icon name={comparedIds.includes(c.id)?'check':'balance'} size={16}/>{comparedIds.includes(c.id)?'비교표에서 빼기':'비교표에 추가'}</button>:<button className={'candidate-save '+(selectedIds.includes(c.id)?'chosen':'')} aria-label={c.name+(selectedIds.includes(c.id)?' 저장 취소':' 후보 저장')} aria-pressed={selectedIds.includes(c.id)} onClick={()=>toggle(c.id)}><Icon name={selectedIds.includes(c.id)?'check':'bookmark'} size={16}/>{selectedIds.includes(c.id)?'저장 취소':'후보 저장'}</button>}</div><CampaignFitSummary creator={c} campaign={campaignOf(brief)}/><div className="match-reason"><span>{story.eligible?'추천 목록에 포함된 이유':candidateStatus(c,brief)}</span><p>{story.summary}</p><small>{story.eligible?c.category+' · 팔로워 '+tierRange(brief.input.sizeTier)+' 조건 충족':story.category}</small></div><div className="match-metrics">{[['평균 조회수',numberText(c.views)+'회','views'],['참여율',c.engagement.toFixed(1)+'%','engagement'],['협업 경험',c.campaigns+'건','experience'],['광고주 평점',c.rating===null?'미평가':c.rating.toFixed(1)+'점','rating']].map(([label,value])=><div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><div className="match-card-bottom"><div><span>참고 협업비</span><strong>{hasKnownBudget(c)?moneyText(c.averageBudget):'견적 문의 필요'}</strong></div><div className="card-actions"><button className="text-button" aria-label={c.name+' 상세 보기'} onClick={()=>setDetail(c)}>캠페인 분석 자세히</button>{<button className="secondary-button" aria-label={c.name+(work[c.id]?' 협업 기록 이어보기':' 문의하기')} onClick={()=>openInquiry(c.id)}>{work[c.id]?'협업 기록 이어보기':'문의하기'}<Icon name="arrow" size={16}/></button>}</div></div></article>;})}</div>{visible.length>limit&&<button className="more-button" onClick={()=>setLimit(n=>n+12)}>후보 더 보기 <span>{Math.min(limit,visible.length)} / {visible.length}</span></button>}</>:<div className="empty-state"><Icon name={view==='saved'?'bookmark':'search'} size={34}/><h2>{view==='saved'?'눈여겨본 후보를 저장하세요.':tab==='quote'?'비용이 미확인된 후보가 없어요.':result.summary.categoryAndTier===0?'이 분야·규모에는 후보가 없어요.':result.summary.overBudget===0?'비용 확인이 필요한 후보만 있어요.':'예산 안에 드는 후보가 없어요.'}</h2><p>{view==='saved'?'후보를 비교하고 문의할 사람을 정리하세요. 한 명에게 바로 문의할 수도 있어요.':tab==='quote'?'추천 후보에서 채널을 살펴보세요.':'아래 조건으로 바꾸거나 다른 분야를 살펴보세요.'}</p>{view==='saved'?<button className="primary-button" onClick={()=>setView('discovery')}>후보 찾기<Icon name="arrow" size={16}/></button>:tab==='quote'?<button className="secondary-button" onClick={()=>setTab('matched')}>추천 후보 보기</button>:<div className="alternatives">{result.alternatives.map((a,i)=><button key={i} onClick={()=>complete({...brief,input:a.input})}><span><strong>{a.label}</strong><small>{a.detail}</small></span><span>{a.count}명 보기 →</span></button>)}{result.needsQuote.length>0&&<button onClick={()=>setTab('quote')}><span><strong>비용 미확인 후보 먼저 보기</strong><small>분야·팔로워 조건 유지</small></span><span>{result.needsQuote.length}명 보기 →</span></button>}<button className="text-button" onClick={()=>setModal('conditions')}>탐색 조건 다시 정하기</button></div>}</div>}
      </>}
    </>}</main></div>
    ...
    {detail&&data&&<Profile creator={detail} all={data} brief={brief} selected={selectedIds.includes(detail.id)} hasWork={!!work[detail.id]} onCampaign={campaign=>setBrief({...brief,campaign})} onInquiry={()=>openInquiry(detail.id)} onToggle={()=>toggle(detail.id)} onClose={()=>setDetail(null)}/>}
    {modal==='compare'&&data&&compared.length>=2&&<Compare work={work} selected={compared} all={data} brief={brief} onClose={()=>setModal(null)} onInquiry={openInquiry}/>}
