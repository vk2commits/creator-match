...
function Profile({creator:c,all,brief,onClose,selected,onToggle,onInquiry,onAnalysis,hasWork}:{creator:Creator;all:Creator[];brief:Brief;onClose:()=>void;selected:boolean;onToggle:()=>void;onInquiry:()=>void;onAnalysis:()=>void;hasWork:boolean}) {
  ...
  return <Modal title="크리에이터 자세히 보기" onClose={onClose}><div className="profile-content"><div className="profile-identity"><Identity creator={c}/><a className="secondary-button" target="_blank" rel="noreferrer" href={channelHref(c.id,DEMO_SESSION)}>채널 보기 ↗</a></div><button className="analysis-launch" onClick={onAnalysis}><span>✦ 캠페인 적합 분석</span><small>우리 제품의 협업 방향과 조율할 점 살펴보기</small><Icon name="arrow" size={18}/></button>
...
export default function App() {
  ...
  const openInquiry=(id:string,proposal?:string)=>{decide(id,'contact');setWork(w=>{
    if(w[id])return w;
    const next=emptyWork(),c=data?.find(x=>x.id===id);
    if(proposal&&brief&&c)next.message=inquiryDraft(c,brief,next,'')+'\n\n'+proposal;
    return {...w,[id]:next};
  });setDetail(null);setModal(null);setAnalysisCreator(null);setWorkflowTarget(id);setView('workflow');};
  ...
  return <div className="app"><a className="skip-link" href="#main">결과로 바로가기</a><aside className="sidebar"><a className="wordmark" href={DEMO_SESSION?'?demo':'./'}><span className="brand-sign">c<span>m</span></span><span>creator<br/>match</span></a><span className="nav-label">캠페인 작업실</span><nav><button className={view==='discovery'?'active':''} onClick={()=>setView('discovery')}><Icon name="search"/>크리에이터 찾기</button><button className={view==='saved'?'active':''} onClick={()=>setView('saved')}><Icon name="bookmark"/>저장한 후보 <span>{selected.length}</span></button><button className={view==='sent'?'active':''} onClick={()=>setView('sent')}><Icon name="send"/>보낸 문의 <span>{sentCount(work)}</span></button><button className={view==='workflow'?'active':''} onClick={()=>setView('workflow')}><Icon name="history"/>협업 관리</button><button className={view==='report'?'active':''} onClick={()=>setView('report')}><Icon name="views"/>성과 리포트</button></nav><div className="sidebar-bottom"><span className="demo-dot"/>Creator Match {storageError&&<p role="alert">저장이 제한되어 새로고침하면 초기화됩니다.</p>}</div></aside>
    ...
    {analysisCreator&&data&&<FitAnalysis key={analysisCreator.id} creator={analysisCreator} all={data} brief={brief} hasWork={!!work[analysisCreator.id]} onClose={()=>setAnalysisCreator(null)} onInquiry={proposal=>openInquiry(analysisCreator.id,proposal)} onCompare={openAnalysis}/>}
